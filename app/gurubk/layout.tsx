...
import { LayoutDashboard, FileCheck, LogOut, Database, MessageCircleQuestionMark, ChevronDown, ChevronRight } from 'lucide-react';
...
function AdminSidebar() {
  ...
  const questionSubItems = [
    { href: '/pakar/question/phq9', label: 'Question PHQ 9' },
    { href: '/pakar/question/gad7', label: 'Question GAD 7' },
  ];
  return (
    <aside className="w-64 bg-white border-r border-slate-200 p-4 flex flex-col">
      <div className="flex items-center space-x-2 mb-8 px-2">
        ...
        <span className="font-bold text-lg">Teacher Panel</span>
      </div>
      ...
      <nav className="flex-1">
        <ul className="space-y-2">
          ...
          {/* Menu Tanya Jawab */}
          <li>
            <Link href="/gurubk/faq" className={`flex items-center p-2 rounded-lg transition-colors ${pathname === '/pakar/faq' ? 'bg-slate-900 text-white' : 'hover:bg-slate-100 text-slate-700'}`}>
