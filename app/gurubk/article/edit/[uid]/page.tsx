...
export default function GurubkEditArticlePage() {
      ...
      try {
        ...
        if (artRes.ok) {
          ...
          if (currentArticle) {
            ...
            setImagePreview(buildApiUrl(`/api/home/articles/${uid}/thumbnail`));
          } else {
  ...
  const getBase64 = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        const encoded = (reader.result as string).split(',')[1] || (reader.result as string);
        resolve(encoded);
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchApi(`/api/article/gurubk/updateArticle/${uid}`, {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        toast.error(json.Message || json.error || 'Gagal memperbarui artikel.');
      }
    } catch {
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        <div className="xl:col-span-3 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-4">
            ...
            {/* Upload Area */}
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-700">Thumbnail Artikel (Opsional — ganti jika ingin diubah)</label>
                ...
                {imagePreview ? (
                  ...
                  <div className="space-y-2 text-center">
                    ...
                    <div className="flex text-sm text-slate-600 justify-center">
                      ...
                      <p className="pl-1 text-slate-500">untuk mengganti yang lama</p>
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-5 sticky top-6">
            ...
            <div className="space-y-3 pt-4">
              <button
                type="submit"
                disabled={loading}
                className="w-full flex justify-center items-center bg-blue-600 text-white p-3 rounded-xl font-bold hover:bg-blue-700 disabled:bg-slate-300 transition-all active:scale-95 shadow-sm"
              >
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Save className="w-4 h-4 mr-2" /> Perbarui Artikel</>}
              </button>
