  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      <div className="flex items-center space-x-4">
        <Link href="/pakar/basisdata/aturan" className="p-2 hover:bg-white rounded-full transition-colors border border-slate-200">
          <ArrowLeft className="w-5 h-5 text-slate-600" />
        </Link>
        <h1 className="text-3xl font-bold text-slate-900">Tambah Aturan Backward Chaining</h1>
      </div>

      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Kode Aturan</label>
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Aturan Backward Chaining</label>
          </div>
          ...
          <div className="flex space-x-3 pt-4 border-t border-slate-100">
            <button type="submit" disabled={loading} className="flex-1 flex justify-center items-center bg-slate-900 text-white p-2.5 rounded-lg font-medium hover:bg-slate-800 disabled:bg-slate-400 transition-colors">
              {loading ? (
                ...
                <>
                  <Save className="w-4 h-4 mr-2" /> Simpan Aturan
                </>
              )}
            </button>
            <Link href="/pakar/basisdata/aturan" className="flex-1 block text-center p-2.5 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors text-sm font-medium">
              Batal
            </Link>
          </div>
        </form>
