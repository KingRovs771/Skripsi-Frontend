...
export default function PakarFaqPage() {
  ...
  const fetchFaqs = async () => {
    ...
    try {
      // API Asumsi karena controller backend Faqs belum ada (Belum dibuat di source Golang)
      const res = await fetchApi('/api/faq/getAllFaqs');
      const json = await res.json().catch(() => ({}));

      // Fallback dummy jika backend gagal merespons agar UI bisa didemonstrasikan
      if (res.ok) {
        setFaqs(json.Data || json.data || []);
      } else {
        // Fallback Dummy Data karena Golang Backend belum ada API Faq
        setFaqs([
          {
            faqs_uid: 'temp-uid-1',
            nisn: '1234567890',
            nama_lengkap: 'Budi Santoso',
            faq_pertanyaan: 'Bagaimana cara mengatasi kecemasan ujian?',
          ...
          },
          {
            ...
            faq_pertanyaan: 'Saya sering merasa sedih tanpa alasan.',
            status: 'Terjawab',
            created_at: '2023-11-21',
          }
        ]);
        toast.info('Menampilkan data dummy. API Backend GET Faqs belum dibuat.', { id: 'fetch-faq' });
      }
    } catch (err) {
      toast.error('Gagal terhubung ke server', { id: 'fetch-faq' });
    } finally {
