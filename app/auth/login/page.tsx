...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchApi('/auth/loginStudents', {
        method: 'POST',
      ...
      // Simpan user_uid jika ada
      const studentUid = data.Data?.student_uid || data.data?.student_uid;
      if (studentUid) {
        localStorage.setItem('student_uid', studentUid);
      }

      toast.success('Login berhasil!');
      router.push(getDashboardUrl('siswa'));
    ...
    <Card className="w-full max-w-md mx-4">
      ...
      <CardContent>
        <form onSubmit={handleLogin} className="grid gap-4">
          ...
          <Button variant="outline" className="w-full" type="button" onClick={() => router.push('/auth/login/admin')}>
            Login Sebagai Admin
          </Button>
          <Button variant="outline" className="w-full" type="button" onClick={() => router.push('/auth/login/pakar')}>
            Login Sebagai Pakar
          </Button>
          <Button variant="outline" className="w-full" type="button" onClick={() => router.push('/auth/login/gurubk')}>
