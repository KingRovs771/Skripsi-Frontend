...
import { Brain, CheckCircle2, AlertTriangle, PlayCircle } from 'lucide-react';
...
export default function DiagnosisIntroPage() {
  ...
  const handleStart = () => {
    // Navigasi ke halaman khusus kuis/kuisioner seperti permintaan
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 fade-in">
      <Card className="rounded-3xl shadow-xl border-none overflow-hidden bg-white">
        <div className="bg-slate-900 p-12 text-center text-white space-y-4">
           ...
           <p className="text-slate-300 font-medium max-w-xl mx-auto leading-relaxed">Pahami kondisi emosional dan psikologis Anda bersama instrumen deteksi sistem pakar resmi kami.</p>
        </div>
        ...
        <CardContent className="p-8 md:p-12 space-y-8">
          <div className="grid md:grid-cols-2 gap-8">
             ...
             <div className="bg-amber-50/50 p-6 rounded-2xl border border-amber-100 flex flex-col justify-center gap-3">
                ...
                <p className="text-sm text-slate-700 font-medium">Sistem pakar (Certainty Factor) ini bertindak sebagai skrining awal, BUKAN diagnosis medis mutlak. Temui psikolog atau Guru BK jika membutuhkan pendampingan lebih lanjut.</p>
             </div>
          </div>
          ...
          <div className="pt-6 flex justify-center border-t border-slate-100">
            <Button 
              onClick={handleStart} 
              className="bg-slate-900 hover:bg-slate-800 text-white text-lg h-14 px-12 rounded-full font-bold shadow-lg shadow-slate-900/20 transition-all hover:scale-105 active:scale-95 flex items-center"
            >
